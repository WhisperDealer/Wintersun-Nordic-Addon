...
5. **Create a FOMOD only if the install has something to ask.** The default for a new release is
   ...
   **If it does need one**, create the stub at `build/releases/<Release Name>/fomod/` — `info.xml`
   and `ModuleConfig.xml`. `build/releases/` is committed source; `build.ps1` copies it into the
   derived `build/staging/<Release Name>/` (wiped and rebuilt every run, gitignored) and packs that.
   Copy the shape from an existing release and keep it minimal; `fomod/` must sit at the **archive
   root**, which `build.ps1` handles. Files that install
   ...
   **If the release ships an installer image**, follow the confirmed-working recipe in CLAUDE.md's
   ...
   baseline (not progressive) JPEG. `-CheckFomod` verifies all three (path resolves, backslashes,
   baseline JPEG) against `build/releases/<Release Name>/`, but it cannot tell you the wizard looks
   right — a wrong layout still builds, so it costs a full install cycle to spot.

6. **Verify before reporting success.** Both must pass:

   ```powershell
   build/build.ps1 -CheckFomod          # manifest <-> ModuleConfig.xml parity
   build/build.ps1                      # full build -> build/dist/<archiveName>.7z
   ```
